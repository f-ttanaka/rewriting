...
depth (V _) = 1
depth (F _ []) = 1
depth (F f ts) = 1 + maximum [depth t | t <- ts]

sortRule :: TRS -> TRS
sortRule trs 
  = sortBy (\(l1,r1) (l2,r2) -> compare (depth l1 + depth r1) (depth l2 + depth r2)) trs

data ProcessResult = Success TRS | Fail TRS

orient' :: Precedence -> [(Term,Term)] -> TRS -> Maybe TRS
orient' _ [] trs = Just trs
orient' p ((s,t):e) trs
  | s == t      = orient' p e trs 
  | gtLpo p s t = orient' p e ((s,t):trs)
  | gtLpo p t s = orient' p e ((t,s):trs)
  | otherwise   = Nothing

orient :: Precedence -> [(Term,Term)] -> Maybe TRS
orient p e = orient' p e []

-- deduce & delete & simplify
deduce :: TRS -> [(Term,Term)]
deduce trs = [(nf trs l, nf trs r) | (l,r) <- cp trs, nf trs l /= nf trs r]

complete' :: Precedence -> TRS -> TRS -> ProcessResult
complete' pr ri []
  | null ei = Success ri
  | otherwise = complete' pr ri ei
  where
    ei = deduce ri
complete' pr ri ei
  | Just trs <- orient pr ei = complete' pr (unionBy sameRules ri trs) []
  | otherwise = Fail []

complete'' :: Precedence -> TRS -> TRS -> ProcessResult
complete'' p ri cps = Fail (nubBy sameRules [(s',t') | (s,t) <- cps,
                                                        let s' = nf ri s,
                                                        let t' = nf ri t,
                                                        s' /= t',
                                                        not (gtLpo p s' t'),
...
complete pr trs = complete' pr trs []
